										...
										<table width="100%" cellpadding="0" cellspacing="0">
											...
											<tr>
												<td data-color="text" data-size="size text" data-min="10" data-max="26" data-link-color="link text color" data-link-style="font-weight:bold; text-decoration:underline; color:#40aceb;" align="center" style="font:16px/29px Arial, Helvetica, sans-serif; color:#888; padding:0 0 21px;">
													...
													@if(count($billdescriptions) > 1)
													<p>Se ha generado este correo dado a que existen cobros prontamente a finalizar. El listado es el siguiente:</p>
													@else
													<p>Se ha generado este correo dado a que existe un cobro prontamente a finalizar. El cobro en cuestión es:</p>
													@endif
													<ul>
														@foreach($billdescriptions as $billdescription)
														<li>{{ $billdescription['description'] }} - Término {{ $billdescription['end_bill']->format('d-m-Y') }} ({{ $billdescription['end_bill']->diffInDays(Carbon\Carbon::today()) }} días restantes)</li>
															@endforeach
														</ul>
														<p>Favor informar a los socios correspondientes de esta situación.</p>
													</td>
												</tr>
											</table>
										</td>
